...
The `config` module is responsible for parsing YAML configuration files into domain models.
...
suite:
  ...
  # defaults — inherited by all configs if not overridden
  testedAgent:
  ...
  configs:
    - "configs/booking-flow.yaml"      # inherits all defaults
    - "configs/cancellation-flow.yaml" # inherits all defaults
    - "configs/edge-cases.yaml"        # overrides judges only
```

**Inheritance rules:**

| Field          | Defined in Suite | Defined in Config | Result                     |
|----------------|------------------|-------------------|----------------------------|
| `testedAgent`  | ✅               | ❌                | Suite value is used        |
| `judges`       | ✅               | ✅                | Config value overrides     |
| `metrics`      | ✅               | ❌                | Suite value is used        |
| `report`       | ❌               | ✅                | Config value is used       |
| `testedAgent`  | ❌               | ❌                | Validation error            |
...
scenarios:
  - name: "indecisive-client"
    description: "A difficult client who cannot decide which day to schedule."
  - name: "angry-client"
    description: "An angry client demanding immediate appointment."
tests:
